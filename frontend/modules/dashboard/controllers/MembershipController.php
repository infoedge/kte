...
use frontend\modules\dashboard\models\SponsorshipSearch;
use frontend\modules\dashboard\models\Dashboard;
use \frontend\modules\dashboard\models\Placement;
//use \yii\data\SqlDataProvider;
use \frontend\modules\dashboard\models\Genealogy;
//use frontend\modules\dashboard\models\Tblcycles;
use frontend\modules\dashboard\models\TblcyclesSearch;
use frontend\modules\dashboard\models\MembershiphistorySearch;
use frontend\modules\dashboard\models\TblpointsSearch;
use frontend\modules\dashboard\models\TblcycleearningsSearch;
use frontend\modules\dashboard\models\TblmatchingSearch;
use frontend\modules\dashboard\models\TblrankearningsSearch;
...
class MembershipController extends Controller
{
    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'only' => ['commissions', 'genealogy', 'myteam','subscribe','upgrade','volumehistory','placement','training','memberprofile','newsfeed','ranks', 'validate-member'],
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['errorpage'],
                    ...
                    [
                        'allow' => true,
                        'actions' => ['commissions', 'genealogy', 'myteam','subscribe','upgrade','volumehistory','placement','training','memberprofile','newsfeed','ranks', 'validate-member'],
                        'roles' => ['@'],
                    ],
    ...
    public function actionComissions()
    {
        $session= Yii::$app->session;
        $memberDetails= Yii::$app->memberdetails;
    ...
    public function actionGenealogy($memberId)
    {
        ...
        if ($membership->load(Yii::$app->request->post())&& $membership->validate() ) {
            
            $themember = $this->validateMember($membership->searchMember)['isMember']==1?$membership->searchMember:$memberId;
        ...
        $memberDetails->prepTree($membership->memberId,$genealogy->parsedArr,$genealogy->aTree,$genealogy->treeList);*/
        
        //get currentmember
        $cm = Yii::$app->userdetails->getPersonId(Yii::$app->user->id);
        $genealogy = $this->populateTree($themember,$cm);
    ...
    public function actionVolumehistory()
    {
        $session= Yii::$app->session;
        $membership = new Membership();
    ...
    public function populateTree($memberId,$currentMember)
    {
        $memberDetails = Yii::$app->memberdetails;
        $genealogy = new Genealogy();
        $genealogy->treeArr= $memberDetails->getTreeDetails($memberId);
        $genealogy->aTree = $memberDetails->getTreeArray($memberId,$genealogy->treeArr,$genealogy->cntrlArr);
        $trialArr = $genealogy->cntrlArr;
        $genealogy->parsedArr = $memberDetails->to_tree($trialArr); //$memberDetails->parseTree($trialArr,$membership->memberId);
        $memberDetails->prepTree($memberId,$genealogy->parsedArr,$genealogy->aTree,$genealogy->treeList,$currentMember);
        return  $genealogy;
    }
    public function validateMember($memno) 
    {
        $result = array();
        $memberDetails = Yii::$app->memberdetails;
        $memberId = $memberDetails->getMemberPartsUsingMemberNo($memno);
        if(empty($memberId)){
            $result['message']='Member Not Found';
            $result['isMember']=-1;
        }elseif(ArrayHelper::isIn($memno,$memberDetails->getAllLeaves($memberId,1,4))){
            $result['message']='Member is a Child';
             $result['isMember']=1;
             //$this->actionGenealogy($memno);
        }else{
            $result['message']=$memno. ' NOT a Child';
            $result['isMember']=0;
        }
        return $result;
    }
